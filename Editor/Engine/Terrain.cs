...
namespace Lab07.Engine
{
    internal class Terrain : ISelectable, IRenderable
    {
        ...
        {
            // Extract pixel data 
            Color[] heighMapData = new Color[HeightMap.Width * HeightMap.Height];
            HeightMap.GetData<Color>(heighMapData);
            // Create heighs[,] array 
            ...
            for (int y = 0; y < Length; y++)
            {
                for (int x = 0; x < Width; x++)
                {
                    ...
                    float amt = heighMapData[y * Width + x].R;
                    // Scale to 0 - 1
                    amt /= 255.0f;
        ...
        private void GenNormals()
        {
            ...
            for (int i = 0; i < IndexCount; i += 3)
            {
                ...
                normal.Normalize();
                // Add the influence off the normal to each vertex in the triangle 
                Vertices[Indices[i]].Normal += normal;
                Vertices[Indices[i + 1]].Normal += normal;
                Vertices[Indices[i + 2]].Normal += normal;
            }

            // Add the influences of the triangles touching each vertex
